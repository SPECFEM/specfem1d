...
! Step !! Warning !! Can create strong oscilations
! if (t<0.d0) then
!   source_time_function = 1.d0
! else if (t >= 0.d0) then
...
! Triangle
!   if ((t >= -hdur) .and. (t < -hdur/2.d0)) then
!     source_time_function = -alpha*t
!   else if ((t >= -hdur/2.d0) .and. (t < 0)) then
!     source_time_function = alpha*t
!   else if (t >= 0.d0) then
!     source_time_function = 0.d0
!   else
